...
@section('title', 'Admin Dashboard - AcademiCore')
  ...
  <div class="space-y-8">
    ...
    <div class="flex items-start justify-between gap-4">
      <div>
        ...
        <p class="text-white/70 mt-2">System overview (all departments)</p>
      </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="bg-black/40 border border-white/10 rounded-2xl p-5">
        ...
        <div class="text-white/60 text-xs mt-2">All departments</div>
      </div>
      ...
      <div class="bg-black/40 border border-white/10 rounded-2xl p-5">
        ...
        <div class="text-white/60 text-xs mt-2">System-wide</div>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
      ...
      <div class="bg-black/40 border border-white/10 rounded-2xl p-5">
        <div class="flex items-center justify-between">
          <div class="font-semibold">Latest Pending Requests</div>
          ...
          @forelse($pendingApprovals as $b)
            <div class="rounded-xl bg-white/5 border border-white/10 p-4">
              ...
              <div class="text-sm text-white/70 mt-1">
                {{ $b->user->name }} • {{ $b->department }} •
                {{ $b->start_at->format('M d, h:i A') }} → {{ $b->end_at->format('h:i A') }}
              </div>
              ...
              <div class="mt-3 flex gap-2">
                ...
                <form method="POST" action="{{ route('approvals.reject', $b) }}">
                  @csrf
                  <input type="hidden" name="decision_note" value="Rejected by admin">
